...
- For Cloud Storage code behind the Firebase Bridge abstraction, use **[@firebase-bridge/cloud-storage](https://www.npmjs.com/package/@firebase-bridge/cloud-storage)** in production and **[@firebase-bridge/storage-mock](https://www.npmjs.com/package/@firebase-bridge/storage-mock)** in tests.
...
```bash
# npm
npm i -D @firebase-bridge/auth-context firebase-functions google-gax

# pnpm
pnpm add -D @firebase-bridge/auth-context firebase-functions google-gax

# yarn
yarn add -D @firebase-bridge/auth-context firebase-functions google-gax
```

> **Peer deps:** `google-gax` is required by the mock Admin Auth API shape. `firebase-functions` is included by this package, but most projects also keep it as a direct dev dependency because tests import v1/v2 function builders from it.
...
Use one manager per suite when you want fast setup, then reset mutable auth state after each test:
...
afterEach(() => {
  authManager.reset(); // keeps registrations, clears runtime mutations
});
```

Use `authManager.clear()` only when a test suite needs to remove registered identities too.
...
- `authManager.https.v1.onCall` / `authManager.https.v2.onCall` for raw callable handlers
- `authManager.https.v1.runCallable` / `authManager.https.v2.runCallable` for wrapped Firebase callable functions
...
Commonly supported operations include:

- token helpers: `createCustomToken()`, `verifyIdToken()`, `createSessionCookie()`, `verifySessionCookie()`
- user lookup: `getUser()`, `getUserByEmail()`, `getUserByPhoneNumber()`, `getUserByProviderUid()`, `getUsers()`, `listUsers()`
- user mutation: `createUser()`, `updateUser()`, `deleteUser()`, `deleteUsers()`, `importUsers()`, `setCustomUserClaims()`, `revokeRefreshTokens()`
- provider config CRUD: `createProviderConfig()`, `getProviderConfig()`, `updateProviderConfig()`, `deleteProviderConfig()`, `listProviderConfigs()`
- action links: `generatePasswordResetLink()`, `generateEmailVerificationLink()`, `generateVerifyAndChangeEmailLink()`, `generateSignInWithEmailLink()`

The mock does not perform real cryptographic verification. JWTs generated by this package are realistic enough for local tests that decode, inspect, or round-trip through the mock API, but they are not valid Firebase production tokens.
...
- real v1/v2 HTTPS function wrappers are executed through their callable or request paths
- auth and App Check data are synthesized into the same high-level shapes Firebase Functions exposes to handlers
- `authorization` and `x-firebase-appcheck` headers are populated with mock JWTs unless explicitly overridden
- mock Admin Auth state is shared with token/context generation, so user mutations affect later invocations
- deterministic `now()` support keeps token timestamps, metadata, and revocation checks stable
- tenant-scoped user stores are supported through `tenantManager().authForTenant()`

Known boundaries:

- JWT signatures are local test artifacts, not production-verifiable Firebase signatures.
- `tenantManager()` only implements `authForTenant()`; tenant CRUD rejects with `auth/operation-not-allowed`.
- `projectConfigManager()` methods reject with `auth/operation-not-allowed`.
- The package is designed for backend tests, not browser/client SDK testing.
...
- mock Admin Auth classes and types such as `BaseAuth` and `Auth`
- identity helpers: `SignInProvider`, `AltKey`, `IdGenerator`
- request/context/token types such as `AuthContextOptions`, `AuthTokenOptions`, `MockIdentity`, `AppCheckData`, `AuthData`
- HTTPS harness types such as `CallableFunctionRequest`, `RawHttpRequest`, `HttpsBroker`, `CallableHandlerV1`, `CallableHandlerV2`, `RequestHandlerV1`, `RequestHandlerV2`
- JWT helpers: `encodeJWT`, `encodeIdToken`, `decodeIdToken`

---

## Troubleshooting

- **My callable/request is unauthenticated**
  Pass `key` in the invocation descriptor and make sure that key was registered, or use `AltKey.uid/email/phone()` to resolve a user from the working set.

- **A user created through `authManager.auth` is not found by registry key**
  Registry keys only apply to identities registered with `authManager.register()`. Use `AltKey.uid()`, `AltKey.email()`, or `AltKey.phone()` for users created through the mock Admin Auth API.

- **State leaks between tests**
  Call `authManager.reset()` after each test to restore registered identities and remove runtime-created users. Call `authManager.clear()` when you need a completely empty manager.

- **`tenantManager().getTenant()` or project config calls fail**
  Those API surfaces exist for compatibility but are intentionally not implemented. Only `tenantManager().authForTenant()` is supported.

- **Token time assertions are flaky**
  Pass a deterministic `now()` function to `new AuthManager({ now })`, or set `iat`, `authTime`, and `expires` per invocation.

---

## Versioning & compatibility

- Runtime dependency: `firebase-functions`
- Peer dependency: `google-gax`
- Node ≥ 18
