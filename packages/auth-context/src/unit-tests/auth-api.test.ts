...
describe('Auth (BaseAuth integration)', () => {
  ...
  describe('tenant-aware Auth', () => {
    it('authManager.auth is an Auth instance and authForTenant returns TenantAwareAuth', () => {
      ...
      const tenantAuth = projectAuth.tenantManager().authForTenant('tenant-1');
    ...
    it('scopes users to their tenant', async () => {
      ...
      const tenantManager = projectAuth.tenantManager();
      const tenantOne = tenantManager.authForTenant('tenant-one');
      const tenantTwo = tenantManager.authForTenant('tenant-two');
  ...
  describe('action links', () => {
    ...
    it('includes tenant in links generated by TenantAwareAuth', async () => {
      ...
      const tenantAuth = authManager.auth
        .tenantManager()
